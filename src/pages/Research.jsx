          ...
          <div key={r.id}
            ...
            <div style={{ padding: "24px 28px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                <Pill T={T} color={T[r.color]} bg={r.color === "green" ? T.greenBg : T.redBg}>{r.type}</Pill>
                <Pill T={T}>{r.date}</Pill>
                <span style={{ marginLeft: "auto", fontSize: 10, color: T.textTer, fontFamily: Fn }}>{r.sector}</span>
              </div>

              <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 8 }}>
                <span style={{ fontSize: 22, fontWeight: 300, color: T.text, fontFamily: Fn }}>{r.name}</span>
                <span style={{ fontSize: 14, fontWeight: 600, color: T.textTer, fontFamily: Fn, letterSpacing: "0.03em" }}>{r.ticker}</span>
              </div>

              <p style={{ fontSize: 13, color: T.textSec, fontFamily: Fn, lineHeight: 1.6, margin: "0 0 16px" }}>{r.tagline}</p>

              <div style={{ display: "flex", gap: 20 }}>
                {r.stats.map((s, i) => (
                  <div key={i}>
                    <div style={{ fontSize: 15, fontWeight: 600, color: T[r.color] || T.text, fontFamily: Fn }}>{s.v}</div>
