...
/* ════════════════════════════════════════════════════════════════ */
export default function ResearchNEE({ T }) {
  ...
  const header = (
    <div style={{ marginBottom: 28 }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 12, marginBottom: 6, flexWrap: "wrap" }}>
        <span style={{ fontFamily: Fh, fontStyle: "italic", fontSize: 36, color: T.text }}>NextEra Energy</span>
        <span style={{ fontSize: 14, fontWeight: 600, color: T.textTer, fontFamily: Fn, letterSpacing: "0.04em" }}>NEE US</span>
      ...
      <div style={{ fontSize: 12, color: T.textTer, fontFamily: Fn, marginBottom: 16 }}>
        NYSE · Juno Beach, Florida · World's largest clean energy company
      </div>
  ...
  const overviewTab = (
    <div>
      {sTitle("What does NextEra Energy do?")}
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 24, marginBottom: 32 }}>
        <Card T={T} style={{ padding: 24 }}>
          <PieChart data={segments.map(s => ({ name: s.name, share: s.share, color: s.color }))} T={T} label="Revenue breakdown by segment (2024)" size={200} />
        </Card>
        <Card T={T} style={{ padding: 24 }}>
          <PieChart data={revenueByRegion.map(r => ({ name: r.name, share: r.share, color: r.color }))} T={T} label="Revenue by geography (approximate)" size={200} />
        </Card>
      </div>
      {sTitle("Geographic detail")}
      <div style={{ display: "grid", gap: 10, marginBottom: 28 }}>
        {revenueByRegion.map((r, i) => (
          <Expandable key={i} title={r.name} subtitle={`~${r.share}% of revenue`} T={T}>{prose(r.description)}</Expandable>
        ))}
      </div>
  ...
  const competitiveTab = (
    <div>
      ...
      {sTitle("US clean energy generation — estimated market share (operating capacity)")}
  ...
  const roleLabels = { own: "NextEra owns this fully", orchestrate: "NextEra orchestrates", partial: "FPL only (not NEER)", support: "NextEra supports", none: "Not involved" };
  const valueChainTab = (
    <div>
      {prose("The electricity industry has four sequential stages: generation, transmission, distribution, and retail. Understanding where NextEra participates in each — and critically, how FPL and NEER differ — reveals the company's unique dual-engine structure.")}
      ...
      <div style={{ padding: "18px 22px", marginTop: 16, borderRadius: T.radius, background: T.text === "#0F172A" ? "rgba(29,78,216,0.04)" : "rgba(96,165,250,0.06)", border: "1px solid " + T.border }}>
        <div style={{ fontSize: 13, color: T.textSec, fontFamily: Fn, lineHeight: 1.75 }}>
          <strong style={{ color: T.deepBlue }}>The key structural insight:</strong> FPL is a 'full stack' utility participating in every stage from generation through retail billing. NEER is a wholesale generator that sells power in bulk and never touches the end consumer. This dual model is what makes NextEra unique — it earns regulated, monopoly-backed returns from FPL while simultaneously competing in wholesale markets through NEER, giving it both stability and growth in a single company.
        </div>
      </div>
    </div>
  );
  ...
  const moatTab = (
    <div>
      ...
      <div style={{ padding: "18px 22px", marginTop: 24, borderRadius: T.radius, background: T.greenBg, border: "1px solid " + T.border }}>
        <div style={{ fontSize: 13, color: T.textSec, fontFamily: Fn, lineHeight: 1.75 }}>
          <strong style={{ color: T.green }}>Why this moat is so difficult to replicate:</strong> A competitor wanting to match NextEra would need to simultaneously own the nation's largest regulated utility (impossible — FPL's franchise is legally exclusive), build the world's largest wind and solar portfolio (a 25-year head start), achieve an A-minus credit rating (requires the stable cash flows from the utility they cannot own), accumulate 145 gigawatts of interconnection queue positions (a two-decade process), and develop proprietary operational software trained on the largest dataset in the industry. No single element is sufficient on its own — the power lies in the combination, and the combination cannot be assembled from scratch.
        </div>
      </div>
    </div>
  );
